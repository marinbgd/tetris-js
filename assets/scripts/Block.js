...
window.TETRIS.Block.prototype.couterRotate = function couterRotate (MAX_LEFT, MAX_BOTTOM) {
    // TODO - improve counter rotation instead of using rotate for 3 times
    var rotatedShape = this.shape[0].map((val, index) => this.shape.map(row => row[index]).reverse())
    rotatedShape = rotatedShape[0].map((val, index) => rotatedShape.map(row => row[index]).reverse())
    rotatedShape = rotatedShape[0].map((val, index) => rotatedShape.map(row => row[index]).reverse())
